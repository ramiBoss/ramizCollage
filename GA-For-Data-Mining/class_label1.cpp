...
#define ATTRIBUTES 18
	...
	for(int i=0;i<pop;i++){
		...
		for(int j=0;j<ATTRIBUTES-1;j++){
			...
			chromosome[i].gene[ATTRIBUTES-1]=1;
	...
	void set_gene_on_off(int p,double onfp,int toggleBits){
        int toggle=0;
		for(int i=0;i<ATTRIBUTES-1;i++)
		{
			if(coin(onfp) == 1){
                if(chromosome[p].flag[i]==true)
                    chromosome[p].flag[i]=false;
                 else
                    chromosome[p].flag[i]=true;
                   toggle++;
			}
			if(toggle == toggleBits)
                break;
		}
	}
	...
	int write_best(){
	ofstream best_file;
	best_file.open("best_chromo1.txt",ios::out |ios::app);
	for(int j=0;j<3;j++){
		for(int i=0;i<ATTRIBUTES;i++){
	...
	*/
    ...
    //const int pop = 5;								// chromosome population size
	const int pop = 100;								// chromosome population size
	//const int gens =2; 								// maximum number of generations
	const int gens =500; 								// maximum number of generations
	const int disc=(int)(ceil(pop*0.9)); 				// chromosomes discarded via elitism
	...
	ofstream best_file;
	best_file.open("best_chromo1.txt", ios::out | ios::trunc);
	best_file.close();
	...
	for(int g=0;g<gens;g++){							 				// How many number of generations
		...
		if(p>pop-disc){													// applying elitism
			...
			if(coin(crp)==1){											// crossover
				...
				//if(coin(onfp)==1)
				set_gene_on_off(p,onfp,toggleBits);
			}
			else{														// mutation

				chromosome[p].mutate(1);
				chromosome[p].fitness=calc_fitness(chromosome[p],tuple);
				//if(coin(onfp)==1)
				set_gene_on_off(p,onfp,toggleBits);
			}
