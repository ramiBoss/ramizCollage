...
#define ATTRIBUTES 18
	...
	for(int i=0;i<pop;i++){
		...
		for(int j=0;j<ATTRIBUTES-1;j++){
			...
			chromosome[i].gene[ATTRIBUTES-1]=2;
   ...
   // cout<<chromo_fit.num<<endl;
    ...
    int write_best(){
	ofstream best_file;
	best_file.open("best_chromo2.txt",ios::out |ios::app);
	for(int j=0;j<3;j++){
		for(int i=0;i<ATTRIBUTES;i++){
	...
	*/
	...
	const int disc=(int)(ceil(pop*0.6)); 				// chromosomes discarded via elitism
	...
	ofstream best_file;
	best_file.open("best_chromo2.txt", ios::out | ios::trunc);
	best_file.close();
